...
| `text_pack_insufficient`        | 422  | `POST /api/rooms`                                | Text pack has fewer items than required for `hand_size × cap + (round_count − 1) × cap`, where `cap` is `config.max_players` (the host's per-room cap, defaulted from the manifest)                                      |
