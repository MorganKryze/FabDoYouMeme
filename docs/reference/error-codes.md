...
| `text_pack_insufficient`        | 422  | `POST /api/rooms`                                | Text pack POOL has fewer items than required for `hand_size × cap + (round_count − 1) × cap`, where `cap` is `config.max_players` and the pool is the SUM across every pack listed for the role (ADR-016)                                      |
| `image_pack_invalid` / `text_pack_invalid` / `prompt_pack_invalid` / `filler_pack_invalid` | 422 | `POST /api/rooms` | One of the role's listed packs has weight ≤ 0 or the same pack appears twice. Surface includes the offending role so the picker can highlight it.                                                                                            |
...
| `invalid_config`           | 422  | `POST /api/rooms`, `PATCH /api/rooms/:code/config` | A field in `config` is out of bounds — covers `round_count`, `joker_count`, `hand_size`, and `max_players` (clamped to `[manifest.min_players, manifest.max_players]`). Error message names the offending field |
