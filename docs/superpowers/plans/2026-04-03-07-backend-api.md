...
- [x] **Step 1: Write the pack handler tests**
...
> **Deviation (implemented):** Removed `var _ = strings.Contains` and `strings` import (not used). `CreatePackParams.OwnerID`, `ListPacksForUserParams.UserID`, and `CreateAdminNotificationParams.PackID/ActorID` use `pgtype.UUID{Bytes: ..., Valid: true}` (not `*uuid.UUID`) as generated by sqlc. `GamePack.OwnerID` is `pgtype.UUID` so nil checks use `.Valid` and `.Bytes` fields instead of pointer dereference.

- [x] **Step 2: Implement `packs.go`**
...
> **Deviation (implemented):** `seedAdmin` was fixed to derive unique usernames from `t.Name()` instead of hardcoded `"admin_pack"`, preventing unique-constraint collisions when multiple tests share the same database.
...
> **Deviation (implemented):** Removed `var _ = strings.Contains` and `strings` import (not used in rooms.go).

- [x] **Step 1: Write `rooms.go`**

- [x] **Step 2: Write `assets.go`**
...
> **Deviation (implemented):** `ListUsersParams.Search` is `*string` (not `Q string`); passed as `&q`. `CountUsers` takes `*string`; passed as `&q`. `CreateInviteParams.CreatedBy` is `pgtype.UUID`; used `pgtype.UUID{Bytes: creatorID, Valid: true}`. `CreateInviteParams.ExpiresAt` is `pgtype.Timestamptz`; added RFC3339 parse + conversion logic. Added `time` and `pgtype` imports.

- [x] **Step 3: Write `game_types.go`**

- [x] **Step 4: Write `admin.go`**
...
> **Deviation (implemented):** Removed the `db interface{...}` field from `WSHandler` — it was unused in `ServeHTTP` and had an invalid interface type. Also added `Registry() *Registry` getter to `game.Manager` (plan called `h.manager.Registry()` but the method didn't exist).

- [x] **Step 1: Write `ws.go`**
...
- [x] **Step 1: Rewrite `main.go` with full wiring**
...
- [x] **Step 3: Smoke test — server starts and health endpoint responds**
...
> **Deviation (implemented):** `SMTP_USERNAME` and `SMTP_PASSWORD` must be non-empty strings (config requires them). Use placeholder values like `"test"` when running locally without a real SMTP server. The postgres container must be running before hitting `/api/health/deep` — the pool connects lazily, so the deep check reflects live reachability.
...
> **Deviation (implemented):** `docs/implementation-status.md` does not exist and was not created (no pre-existing file to update). `mw.Session()` requires a `logger *slog.Logger` second argument — passed `logger` in the call.
