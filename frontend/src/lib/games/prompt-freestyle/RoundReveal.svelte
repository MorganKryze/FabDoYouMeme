<script lang="ts">
  ...
  let {
    ...
    /** Optional. ResultsView in the live room doesn't plumb the round
     *  through; in that case we fall back to room.currentRound for the
     *  prompt's prefix/suffix. ReplayRoundView passes a synthesised Round. */
    round?: Round | null;
  } = $props();

  const sourceRound = $derived(round ?? room.currentRound);
  const prefix = $derived(
    sourceRound ? (sourceRound.item?.payload as { prefix?: string } | undefined)?.prefix ?? '' : ''
  );
  const suffix = $derived(
    sourceRound ? (sourceRound.item?.payload as { suffix?: string } | undefined)?.suffix ?? '' : ''
  );
  ...
  // The submission's filler value lives under either `filler` (live) or
  // `payload.filler` (replay) — accept both shapes here so the same component
  // serves both call sites.
  function fillerOf(s: Submission): string {
    const f = (s as Submission & { filler?: string }).filler;
    ...
    >
      {#each podium as entry, i (entry.sub.id + '-' + entry.place + '-' + i)}
        ...
        >
          ...
          <div
            class="w-full rounded-[12px] border-[2.5px] border-brand-border-heavy bg-brand-surface px-3 py-3 text-brand-text leading-snug text-balance flex-1"
            style="box-shadow: inset 0 2px 0 rgba(0,0,0,0.04);"
          >
            <SentenceWithBlank {prefix} {suffix} filler={fillerOf(entry.sub)} size="sm" />
          </div>
  ...
  <div use:reveal={{ delay: 4 }} class="flex flex-col gap-2.5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {#each alsoRans as sub (sub.id)}
        ...
        >
          <div
            class="w-full rounded-[12px] border-[2.5px] border-brand-border-heavy bg-brand-white px-3 py-2 text-sm text-brand-text leading-snug"
          >
            <SentenceWithBlank {prefix} {suffix} filler={fillerOf(sub)} size="sm" />
          </div>
