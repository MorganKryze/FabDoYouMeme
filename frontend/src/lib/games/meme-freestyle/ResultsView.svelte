<script lang="ts">
  ...
  // Tie-aware podium shape. A tie at the top becomes a duet (no 3rd place).
  ...
  const podium = $derived.by(() => {
    if (podiumShape === 'trio') {
      // Display order 2nd · 1st · 3rd so first place sits centered.
      return [
...
<div class="flex flex-col gap-6">
      ...
      >
        {#each podium as entry, i (entry.sub.id + '-' + entry.place + '-' + i)}
          ...
          <div
            ...
            "
          >
            <!-- Points ribbon — 1st only in trio -->
            {#if isFirst && points > 0}
              <span
  ...
  <!-- Also-rans (only when podium is a full trio and there are extras) -->
