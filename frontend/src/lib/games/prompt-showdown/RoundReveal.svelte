<script lang="ts">
  ...
  // Filler text travels under either `text` (live) or `payload.text` (replay).
  function textOf(s: Submission): string {
    const t = (s as Submission & { text?: string }).text;
    ...
    >
      {#each podium as entry, i (entry.sub.id + '-' + entry.place + '-' + i)}
        ...
        >
          ...
          >
            <SentenceWithBlank {prefix} {suffix} filler={textOf(entry.sub)} size="sm" />
          </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {#each alsoRans as sub (sub.id)}
        ...
        >
          ...
          >
            <SentenceWithBlank {prefix} {suffix} filler={textOf(sub)} size="sm" />
          </div>
