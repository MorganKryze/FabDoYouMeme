<script lang="ts">
  ...
  function vote() {
    ...
    ws.send('prompt-showdown:vote', { submission_id: selectedId });
    voted = true;
  }
</script>
...
<div class="flex flex-col gap-3 md:gap-6 pb-28 md:pb-0">
  ...
  >
    ...
    <div class="relative flex items-center justify-between gap-2">
      <span class="text-[10px] font-bold uppercase tracking-[0.25em] opacity-70">
        {m.game_prompt_showdown_vote_title()}
      </span>
      <span
      ...
      >
        {m.game_prompt_showdown_submissions_in({ count: submissions.length })}
      </span>
    </div>
    <p class="relative m-0 leading-tight tracking-tight">
      <SentenceWithBlank {prefix} {suffix} placeholder={m.game_prompt_showdown_blank_placeholder()} size="lg" />
    </p>
    <span class="relative text-[11px] font-bold uppercase tracking-[0.2em] opacity-70 mt-auto">
      {m.game_prompt_showdown_one_vote()}
    </span>
  </div>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    {#each submissions as sub, i (sub.id)}
      ...
      {@const fillerVal = (sub as Submission & { text?: string }).text ?? ''}
      <button
        ...
        aria-label={m.game_prompt_showdown_filler_aria({ letter, filler: fillerVal })}
      ...
      >
        ...
        {#if voted && isSelected}
          ...
          >
            {m.game_prompt_showdown_voted_chip()}
          </span>
        {/if}
  ...
  {#if room.ownSkippedVote}
    <p class="text-center text-sm font-bold text-brand-text-mid m-0">
      {m.game_prompt_showdown_skipped_waiting()}
    </p>
  {:else if !voted}
    <div class="hidden md:flex flex-row items-center justify-center gap-3">
      ...
      >
        ...
        {m.game_prompt_showdown_lock_vote()}
      </button>
  ...
  {:else}
    <p class="text-center text-sm font-bold text-brand-text-mid m-0">
      {m.game_prompt_showdown_voted_waiting()}
    </p>
  {/if}
</div>
